...
class CloudEnvironment:
    def __init__(self):
        ...
        self.tenant_resources: dict[str, list[ComputeResource]] = {}
    ...
    def allocate(
        ...
        resource = self.resource_pool.allocate(cpu=cpu, memory=memory)
        self.tenant_resources[tenant_id].append(resource)
        return resource

    def release(self, tenant_id: str, resource_id: str) -> None:
        if tenant_id not in self.tenants:
            raise ValueError("Unknown tenant")
    ...
    def resources_for_tenant(self, tenant_id: str) -> list[ComputeResource]:
        if tenant_id not in self.tenants:
            raise ValueError("Unknown tenant")

        return list(self.tenant_resources[tenant_id])
